...
These are the process and organisational terms used throughout this section's pages (feedback, requirements, liaison statements, work-item and CR tracking) — the vocabulary of _how_ standards get made, as distinct from the technical A-Z below.
...
An IETF/W3C protocol stack for interactive, low-latency peer-to-peer media and data: RTP with RTCP feedback secured as DTLS-SRTP, ICE for connectivity, and data channels (SCTP over DTLS). Used by 3GPP's RTC work; see [Standards: Real-Time Communications (RTC)](/tech/standards/rtc).
