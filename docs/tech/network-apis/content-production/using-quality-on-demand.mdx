...
<figure>
  <img loading="lazy" src={useBaseUrl('/img/tech/network-apis/content-production/figure_qualityondemand.png')} width="80%" alt="High-level architecture for Quality on Demand, showing the media application (ASP) invoking the Network API Platform to create a QoS session between a device and an application server." />
