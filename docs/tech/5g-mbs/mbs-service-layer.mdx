...
Checked directly against TS 33.501 (Release 20, the latest available): Clause W.4.1.3 confirms the mechanism in more detail — the MBSSF "takes the role of the BM-SC" from the legacy MBMS security architecture (TS 33.246), and the UE authenticates to the MBSSF using either **GBA** (Generic Bootstrapping Architecture, as in MBMS security) or **AKMA** (Authentication and Key Management for Applications, TS 33.535): when AKMA is used, the MRK (the UE-to-MBSSF authentication key) is derived from KAF, and the MUK (which protects MSK delivery) is set to KAF — KAF itself being an AKMA Application Key derived per-AF from the AKMA anchor key, not the anchor key directly. The MBS traffic-encryption key (MTK) is generated and distributed separately, by the control-plane procedure in TS 33.501 Clause W.4.1.2.
:::
