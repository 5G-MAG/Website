...
| 7. Demodulate MTCH via PDSCH | MAC, PHY | Shared G-RNTI table entry; **distinct** LCID source (generic Table 6.2.1-1); **distinct DCI format 4_1** — unlike Step 4, this genuinely differs from broadcast's 4_0 (TS 38.213/38.214, verified) | **TS 38.321** Clause 5.3.1/5.3.2 in full (the multicast MTCH MAC reading rule is inferred, not directly quotable — see the caveat below); **TS 38.214** Clause 5.1.2 in full, including resolving 4_1 vs. the also-mentioned 4_2 (open detail, not resolved on this page) |
...
- **PDSCH resource allocation** in **[3GPP TS 38.214](https://www.3gpp.org/dynareport/38214.htm) Clause 5.1.2**: "For the PDSCH scheduled by PDCCH with DCI format 4_0 with CRC scrambled by Multicast MCCH-RNTI, the parameter `pdsch-TimeDomainAllocationList`, `mcs-Table`, `xOverhead`, `rateMatchPatternToAddModList` and `RateMatchPatternLTE-CRS` are provided by `pdsch-ConfigMCCH`" for the MCCH case, with a separate `pdsch-ConfigMulticast`-driven path for the MTCH/DCI-4_1 case.
- **DCI format 4_1 vs. 4_2, resolved:** TS 38.214 also mentions **DCI format 4_2** alongside 4_1 for "G-RNTI for multicast" — the split follows the HARQ-ACK feedback path, not the delivery mode directly. **[3GPP TS 38.213](https://www.3gpp.org/dynareport/38213.htm)** ties DCI format 4_2 specifically to `pucch-ConfigMulticast1` ("if the UE is configured to monitor PDCCH for DCI format 4_2, [...] is provided by `pucch-ConfigMulticast1`") — PUCCH-based HARQ-ACK feedback, which only exists in RRC_CONNECTED. DCI format 4_1, by contrast, is the format explicitly named for `searchSpaceMulticastMTCH` "for PDCCH receptions in RRC_INACTIVE state" (Clause 10.1, already cited above), and is also the format used generally for multicast SPS PDSCH release regardless of RRC state (TS 38.214, e.g. "a multicast SPS PDSCH release indicated by DCI format 4_1"). So: **DCI format 4_1** is this page's format (no PUCCH feedback exists in RRC_INACTIVE); **DCI format 4_2** belongs to the Release 17 RRC_CONNECTED multicast case on a future page, not this one. (TS 38.212, which formally defines DCI field contents, was not available to double-check this from the format-definition side rather than the procedural side.)
...
Every step of the acquisition path (0–7) has now been checked directly against 3GPP specification documents: TS 38.331 (V19.3.0) for RRC, TS 37.324 (V19.0.0) for SDAP, TS 38.321/TS 38.322 (both V19.3.0) for MAC/RLC, TS 38.213/TS 38.214 (both V19.4.0) for the PHY layer, and TS 38.323 (V19.1.0) for PDCP — every layer on this page is now checked against its current published version. Two real findings came out of this check. First: the multicast-inactive case uses its own **Multicast MCCH-RNTI (FFFB)**, not the broadcast MCCH-RNTI (FFFD) as this page previously stated — TS 38.321 defines the two as separate values in Table 7.1-1, even though they share the same MCCH LCID (0) once decoded. Second, confirmed at the PHY layer: multicast-inactive **MCCH** shares broadcast's DCI format 4_0 (only the RNTI differs), but multicast-inactive **MTCH** uses a genuinely different DCI format, 4_1, not shared with broadcast MTCH's 4_0. RLC (TS 38.322) has no multicast-specific content at all and applies identically to broadcast. One remaining soft spot: TS 38.321 Clause 5.3.1 states the broadcast MTCH reading rule as an explicit quote ("PDCCH for the G-RNTI configured for broadcast MTCH"), but does not give an equally explicit quotable rule for multicast MTCH reading at the MAC layer — the mechanism is confirmed at the PHY layer (DCI format 4_1) and inferred at MAC from the shared Table 7.1-1 G-RNTI entry, not from a directly quotable MAC-layer sentence. TS 38.214 also mentions DCI format 4_2 for "G-RNTI for multicast" in some contexts; this has since been resolved (see Step 7's PHY section) — 4_2 requires `pucch-ConfigMulticast1` (PUCCH feedback, RRC_CONNECTED only) and so belongs to the Release 17 multicast case, not this page, though this was confirmed from TS 38.213's procedural text rather than TS 38.212's formal DCI format definitions (not available this session).
:::
