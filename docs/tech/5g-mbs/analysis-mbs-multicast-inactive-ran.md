...
- **PDSCH resource allocation** in **[3GPP TS 38.214](https://www.3gpp.org/dynareport/38214.htm) Clause 5.1.2**: for the MCCH case (DCI format 4_0, CRC scrambled by Multicast MCCH-RNTI), the resource-allocation parameters are provided by `pdsch-ConfigMCCH`; the MTCH/DCI-4_1 case instead uses `pdsch-ConfigMTCH-r18` if configured, falling back to `pdsch-ConfigMCCH` otherwise (Clause 5.1.2.1). `pdsch-ConfigMulticast` is a separate field governing the Release 17 RRC_CONNECTED multicast case, not this RRC_INACTIVE one.
