...
- **Multicast MBS session**: An MBS session to deliver a multicast communication service. A multicast MBS session is characterised by the content to send, the list of UEs that may receive the service and optionally by a geographical area over which to distribute it.
  Broadcast and multicast MBS sessions are time-bound, but their life-cycles are not documented symmetrically in TS 23.247: broadcast's provisioning phases (creation, service announcement, data transfer, release/deletion) are in **Clause 4.2.2**, while multicast has both its own provisioning phases (**Clause 4.2.1**) and a dedicated **Clause 4.3 "Multicast session state model"** with no broadcast counterpart — broadcast has no equivalent numbered state-model clause.
...
<figure>
  <img loading="lazy" src={useBaseUrl('/img/tech/5g-mbs/figure_mbs_8.png')} alt="MBS traffic delivery for a multicast session, showing shared and individual delivery from the MB-UPF towards the NG-RAN and UEs" />
  <figcaption><em>MBS traffic delivery methods (multicast), adapted from TS 23.247 Figure 4.1-1.</em></figcaption>
</figure>

<figure>
  <img loading="lazy" src={useBaseUrl('/img/tech/5g-mbs/figure_mbs_9.png')} alt="MBS traffic delivery for a broadcast session, showing a single shared copy delivered from the MB-UPF to each NG-RAN node" />
...
| Transport towards the RAN     | Shared GTP-U tunnel at reference point N3mb     | Per-UE PDU session (via the UPF at N19mb, then N3)                 |
...
Where the acronyms above mean: GTP-U (GPRS Tunnelling Protocol, User Plane — [TS 29.281](https://www.3gpp.org/dynareport/29281.htm) — the protocol carried over N3mb/N3; only the user-plane variant applies here, not GTP-C); NG-RAN (Next Generation Radio Access Network); MNO (Mobile Network Operator); gNodeB (5G base station); UPF (User Plane Function).
...
- **5GC shared MBS traffic delivery method**: 5G CN receives a single copy of MBS data packets and delivers a single copy of those MBS data packets to all relevant RAN nodes.
  ...
  - MBS packet is forwarded by the MB-UPF directly to the relevant NG-RAN nodes at their N3mb reference point via a shared GTP-U tunnel.
...
The two session types differ in how a UE becomes a receiver, which is why their life-cycles are specified separately: broadcast in Clause 4.2.2, multicast across Clause 4.2.1 and Clause 4.3.
...
Checked directly against TS 29.244 V19.6.0, TS 29.518 V19.7.0, TS 29.532 V19.3.0 and TS 29.537 V19.3.0 (all Release 19, the current published versions): every core-network claim on this page is confirmed correct. N4mb is a genuine PFCP-based reference point defined for MB-SMF↔MB-UPF configuration (TS 29.244 Clause 5.34.2). Broadcast context management does use a dedicated Namf service — `Namf_MBSBroadcast`, "to create, update or release a broadcast MBS session context in the AMF" (TS 29.518 Clause 5.6). TMGI allocation and MBS session management (`Nmbsmf_TMGI`, `Nmbsmf_MBSSession`) are in TS 29.532, applicable to both broadcast and multicast. PCF multicast/broadcast policy (`Npcf_MBSPolicyControl`) is in TS 29.537.

TS 23.247 V18.8.0 confirms the life-cycle placement above: the table of contents lists "4.2.1 Multicast data provisioning", "4.2.2 Broadcast data provisioning" and "4.3 Multicast session state model", with no Clause 4.4 broadcast-state-model counterpart.
:::
