...
<h1>Content Delivery Protocols</h1>
</div>
</div>

<div class="topic-lead">
FLUTE and ROUTE transport protocols for delivering DASH, HLS and CMAF media over 5G broadcast and multicast networks, one-way and without a return channel.
</div>

## Overview

Content delivery over broadcast and multicast networks relies on transport protocols that can deliver files and media objects one-way, without a return channel from each receiver. This page covers the reference tooling and technical resources for those protocols, in particular File Delivery over Unidirectional Transport (FLUTE) and Real-time Object delivery over Unidirectional Transport (ROUTE), which underpin 5G-MAG's broadcast and multicast work. 5G-MAG maintains open-source implementations such as the FLUTE library used in the 5G Broadcast tools. For acronyms used here, see the [Glossary](/tech/glossary).

**Key specifications:** IETF [RFC 3926](https://www.rfc-editor.org/rfc/rfc3926) (FLUTE version 1, the profile 3GPP mandates) and [RFC 6726](https://www.rfc-editor.org/rfc/rfc6726) (FLUTE version 2, reference only for 3GPP use), [RFC 9223](https://www.rfc-editor.org/rfc/rfc9223) (ROUTE), 3GPP [TS 26.346](https://www.3gpp.org/dynareport/26346.htm) (MBMS, FLUTE file delivery).

**Reference tools:** the [Content Delivery Protocols reference tools](/reference-tools/content-delivery) implement FLUTE and ROUTE for the delivery scenarios described below.
...
<p>FLUTE and ROUTE transport building blocks (LCT, ALC, FEC), and how DASH/HLS/CMAF formats are carried over each, below.</p>
</div>
</div>
...
<div class="godeeper-card__body">
<p>IETF and 3GPP specifications for FLUTE, ROUTE and the media formats delivered over them.</p>
<ul class="godeeper-card__links">
<li><a href="/standards/content-delivery">Standards: Content Delivery Protocols</a></li>
</ul>
</div>
</div>
...
<ul class="godeeper-card__links">
<li><a href="/reference-tools/content-delivery">Content Delivery Protocols</a></li>
</ul>
</div>
</div>
...
The slide deck below introduces the content delivery protocols in scope; download the file for the full detail.

<div class="pdf-embed-wrapper">
  <iframe loading="lazy" class="pdf-embed" src={useBaseUrl('/docs/Reference_Tools_Multimedia_delivery_protocols.pdf')} title="Content Delivery Protocols reference tools overview slide deck"></iframe>
</div>
...
- **LCT** (Layered Coding Transport) defines the common packet header, session and transport-object identification, and congestion-control signalling used by both protocols. Each packet carries a Transport Session Identifier (TSI) and a Transport Object Identifier (TOI). Current IETF text is RFC 5651; the profile 3GPP actually mandates (see the note below) uses its predecessor, RFC 3451.
- **ALC** (Asynchronous Layered Coding) is the protocol instantiation that combines LCT with an FEC building block and, optionally, layered congestion control. It gives massively scalable one-way delivery: any number of receivers can join without the sender knowing about them. Current IETF text is RFC 5775; the 3GPP-mandated profile uses its predecessor, RFC 3450.
...
## FLUTE (RFC 3926, version 1)

:::note[Which FLUTE version applies]
Two generations of FLUTE exist. **RFC 3926 specifies version 1**, and that is the
profile 3GPP mandates: TS 26.517 clause 6.2.1 binds the Object Distribution Method to
the MBMS Download Profile of TS 26.346, whose clause 7.2.0 requires RFC 3926 together
with RFC 3450 (ALC) and RFC 3451 (LCT). RFC 6726 revises FLUTE as version 2 and is
current IETF text, but it is not what a 3GPP deployment implements. See
[Standards: Content Delivery Protocols](/standards/content-delivery) for the full chain.
:::
...
| Aspect                  | FLUTE (RFC 3926 v1)                     | ROUTE (RFC 9223)                                        |
| ----------------------- | --------------------------------------- | ------------------------------------------------------- |
...
- [Standards: Content Delivery Protocols](/standards/content-delivery): the specification list for this topic
- [Content Delivery Protocols Reference Tools](/reference-tools/content-delivery): the software implementation
- [5G Broadcast - TV, Radio and Emergency Alerts](/tech/5g-broadcast) and [Multicast & Broadcast in 5G](/tech/5g-mbs): the delivery systems these transports run over
