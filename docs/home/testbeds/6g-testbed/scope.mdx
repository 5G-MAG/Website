...
<img loading="lazy" src={useBaseUrl('/assets/images/6g-testbed/architecture.png')} style={{width: '70%'}} alt="6G Testbed architecture: an AI Traffic Characterization framework (orchestrator, scenarios, clients, capture, analysis) running on top of the shared Network Emulator, which applies impairments to the network interface." />
...
## How this relates to standardisation
...
- **The AI workloads produce traffic characterisation for ongoing 3GPP study work.** The scenario set (chat, agentic AI, image, video, multimodal, real-time) and the metrics the testbed computes (time-to-first-token, total latency, uplink/downlink ratios, token rates, agent loop factors) are intended as measured inputs to the AI, media and 6G studies currently running in 3GPP, rather than to any single one of them. The testbed is not scoped to one study item, and which studies it feeds changes as that work progresses.
...
- Standards: [6G Media](/standards/6g) and [AI/ML in 5G Media](/standards/ai-ml) — the study and specification landscape this work feeds into
- Tech: [Towards 6G Media](/tech/6g): the research analysis
