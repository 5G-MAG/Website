...
The implementation (a management portal covering the full Dedicated Networks lifecycle: service areas, profiles, network creation and monitoring, device access, webhooks) lives on [rt-camara-examples](https://github.com/5G-MAG/rt-camara-examples)'s `development` branch, not `main` — check that branch specifically when verifying this claim against the source.
