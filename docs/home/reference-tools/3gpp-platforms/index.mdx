---
hide_title: true
sidebar_class_name: project-index-link
title: 3GPP RAN and Core Platforms
sidebar_position: -1
description: Reference platform tools for deploying a lab 5G RAN and Core network, built on Open5GS and srsRAN.
---
import CommunityStats from '@site/src/components/CommunityStats';
import ProjectContributors from '@site/src/components/ProjectContributors';
import ProjectVideos from '@site/src/components/ProjectVideos';
import useBaseUrl from '@docusaurus/useBaseUrl';
import youtubePlaylists from '@site/static/data/youtube-playlists.json';

export const projectVideos = (youtubePlaylists.projects?.['3gpp-platforms']?.videos || []).slice(0, 1);

<div class="topic-banner">
<div class="topic-banner__icon-wrap">
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path stroke="none" d="M0 0h24v24H0z" fill="none" />
  <path d="M11.482 20.924a1.666 1.666 0 0 1 -1.157 -1.241a1.724 1.724 0 0 0 -2.573 -1.066c-1.543 .94 -3.31 -.826 -2.37 -2.37a1.724 1.724 0 0 0 -1.065 -2.572c-1.756 -.426 -1.756 -2.924 0 -3.35a1.724 1.724 0 0 0 1.066 -2.573c-.94 -1.543 .826 -3.31 2.37 -2.37c1 .608 2.296 .07 2.572 -1.065c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.312 .318 1.644 1.794 .995 2.697" />
  <path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0" />
  <path d="M20 21l2 -2l-2 -2" />
  <path d="M17 17l-2 2l2 2" /></svg>
</div>
<div class="topic-banner__text">
<span class="topic-banner__kicker">Reference Tools</span>
<h1>3GPP RAN and Core Platforms</h1>
</div>
</div>

<div class="topic-lead">
Reference platform tools for deploying 5G Radio Access Network (RAN) and Core network components in a lab or testbed environment.
</div>

<div style={{margin: '8px 0'}}><a class="button button--outline button--primary" href={useBaseUrl('/reference-tools/3gpp-platforms/scope')} style={{margin: '2px 4px 2px 0'}}>Scope</a> <a class="button button--outline button--primary" href={useBaseUrl('/reference-tools/3gpp-platforms/resources')} style={{margin: '2px 4px 2px 0'}}>Resources</a> <a class="button button--outline button--primary" href={useBaseUrl('/reference-tools/3gpp-platforms/tutorials')} style={{margin: '2px 4px 2px 0'}}>Tutorials</a> <a class="button button--outline button--primary" href={useBaseUrl('/reference-tools/3gpp-platforms/tutorials#developer-exchange')} style={{margin: '2px 4px 2px 0'}}>Developer Exchange</a></div>

The project provides configurations, scripts, and integration guides for standing up 3GPP-compliant RAN and Core (5G Core, 5GC) stacks, forming the network infrastructure layer on which the other 5G-MAG reference tools run. It is useful for developers who need a complete 5G system to validate end-to-end media delivery pipelines. 5G-MAG builds these platforms on established open-source projects (Open5GS for the core, srsRAN for the radio) so members can reproduce a working network without commercial equipment.

**Specifications referenced:** 3GPP [TS 38.300](https://www.3gpp.org/dynareport/38300.htm) (NR overall description), [TS 23.501](https://www.3gpp.org/dynareport/23501.htm) (5G System architecture), [TS 38.401](https://www.3gpp.org/dynareport/38401.htm) (NG-RAN architecture and interfaces).

:::tip[Where to start]
- **Scope** for which RAN and Core components 5G-MAG builds, and how the srsRAN and Open5GS forks split into MBS and 5G Broadcast branches — see [Scope](./scope).
- **Resources** for the source code, repositories and latest releases — see [Resources](./resources).
- **Tutorials** for the walkthrough to stand up a working 5G network with Open5GS and srsRAN, plus the SDR hardware reference and Developer Exchange — see [Tutorials](./tutorials).
:::

## Contributors

<ProjectContributors name="3GPP RAN and Core Platforms" />

## Project Overview

The slide deck below introduces the 3GPP RAN and Core Platforms project, its components, and how they fit together. In short: it packages an Open5GS-based 5G core and an srsRAN-based radio access network into a lab-ready 5G system that the other reference tools can run against.

<div class="pdf-embed-wrapper">
  <iframe loading="lazy" class="pdf-embed" src={useBaseUrl('/docs/Reference_Tools_3GPP_Platforms.pdf')} title="3GPP RAN and Core Platforms: project overview slide deck"></iframe>
</div>

<a class="button button--outline button--primary" href={useBaseUrl('/docs/Reference_Tools_3GPP_Platforms.pdf')} style={{margin: '4px 0'}}>Download the slide deck of this Project</a>

---

## Go deeper

| Area                                  | Details                                                                                                                                                            |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Technical documentation**           | RAN and Core aspects of broadcast delivery: [Multicast & Broadcast in 5G on the Tech portal](/tech/5g-mbs)                                                          |
| **Standards**                         | The specifications these forks are maintained against: [5G Multicast & Broadcast Services](/standards/5g-mbs), [5G Broadcast](/standards/5g-broadcast)              |
| **Projects built on these platforms** | [5G Multicast & Broadcast Services](../5g-mbs), [5G Broadcast](../5g-broadcast), [5G Core Service Consumers](../5g-core)                                            |

<div class="onair-kicker"><span class="onair-kicker__dot"></span>In Action</div>

## Developer Exchange

<ProjectVideos videos={projectVideos} name="3GPP RAN and Core Platforms" />

## Related

- [Scope](./scope): what is in scope and the upstream software the platforms build on
- [Resources](./resources): the forks and branches, mapped to their use cases
- [Tutorials](./tutorials): step-by-step setup guides

## Community Stats

<CommunityStats name="3GPP RAN and Core Platforms" />
