...
hide_title: true
sidebar_position: 5
description: Shows how to log MBMS Modem signal-quality measurements to a CSV file, optionally tagged with GPS position and time.
---
...
_MBMS Modem_ relies on GPSD (https://gpsd.gitlab.io/gpsd/) for GPS data acquisition. GPSD is the GPS daemon that reads the receiver and makes position and time available to other programs.
...
- Adding gpsd to the _dialout_ group: ` sudo usermod -a -G dialout gpsd`
