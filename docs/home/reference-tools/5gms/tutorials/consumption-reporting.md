...
Now that a provisioning session has been created, a Consumption configuration can be added to that provisioning
...
Now that a Consumption Reporting configuration has been provided via the `M1` interface, the Service Access Information
...
Application Function, the client automatically creates and sends Consumption Reports.
...
<div><strong>You now have Consumption Reporting running end-to-end.</strong> The client generates consumption reports while streaming, sends them to the AF over M5, and you can read the resulting reports directly from disk.</div>
</div>
