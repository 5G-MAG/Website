...
Note that some fields are accepted by the API but are not yet implemented by the Reference Tools (see the `:::note` in the ["Summary of APIs"](#summary-of-apis-supported-for-5g-media-streaming) section below): Content Preparation, Edge Resources, Geo-fencing and URL signing.
