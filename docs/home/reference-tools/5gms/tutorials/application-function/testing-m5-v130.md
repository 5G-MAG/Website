...
The M5 interface is the media session handling interface: a client on the User Equipment (UE) fetches Service Access Information from the 5G Media Streaming downlink (5GMSd) Application Function (AF), which tells the client where and how to access the media. This page verifies that behaviour for version v1.3.0 and above. See the [Glossary](/tech/glossary) for wider 5GMS terminology.

:::note[Version]
This page covers the 5GMSd Application Function v1.3.0 and above. For earlier versions see [Testing M5 (v1.0.0 to v1.1.x)](./testing-m5-v100) and [Testing M5 (v1.2.x)](./testing-m5-v120).
:::

:::note[Response structure changed in v1.3.0]
From v1.3.0 the Service Access Information response no longer uses a single `mediaPlayerEntry` string. Instead, `streamingAccess` contains an `entryPoints` array, each with a `locator` (the media URL) and a `contentType`. The earlier version pages show the older `mediaPlayerEntry` form.
:::

## Prerequisites

- A built and installed 5GMSd Application Function (see [local user building and installation](./installation-local-user-5GMSAF)).
- The example configurations from the `examples` directory.
...
See [Configuring the Application Function](./configuration-5GMSAF) for more details on Application Function
configuration.
...
`distributionConfigurations.domainNameAlias` properties then the entry point `locator` in the ServiceAccessInformation will use the
...
1. Using the provisioning session id, returned from the `m1-session` command, perform a GET request on the interface URL.
   ...
   Example command if the provisioning session id is f863831c-daa8-41ed-862b-c1f4c44bccf3:

   ```bash
   curl -v 'http://127.0.0.24:7777/3gpp-m5/v2/service-access-information/f863831c-daa8-41ed-862b-c1f4c44bccf3'
   ```
   ...
   **Note:** There may also be additional HTTP library information lines output by curl starting with a `*` character interspersed with the output.
   ...
   `provisioningSessionType` will be "DOWNLINK". The entry point `locator` will be a "http://" URL for the canonical name set in the
...
for the entry point `locator` in the ServiceAccessInformation.
...
1. Using the provisioning session id, returned from the `m1-session` command, perform a GET request on the interface URL
   ...
   **Note:** There may also be additional HTTP library information lines output by curl starting with a `*` character interspersed with the output.
   ...
   The HTTP response is a 200 status code with the body containing a ServiceAccessInformation JSON object. The
   `provisioningSessionId` in the ServiceAccessInformation object should match the id presented in the request URL. The
   `provisioningSessionType` will be "DOWNLINK". The entry point `locator` will be a "https://" URL (to show the AF is giving
...
#### Testing the entry point `locator` when providing a `domainNameAlias`

If the ContentHostingConfiguration given in the `msaf.yaml` configuration file includes a
`distributionConfigurations.domainNameAlias` then this hostname will be used as the authority for the entry point `locator` in the
...
1. Using the provisioning session id, returned from the `m1-session` command, perform a GET request on the interface URL
   ...
   **Note:** There may also be additional HTTP library information lines output by curl starting with a `*` character interspersed with the output.
   ...
   The HTTP response is a 200 status code with the body containing a ServiceAccessInformation JSON object. The
   `provisioningSessionId` in the ServiceAccessInformation object should match the id presented in the request URL. The
   `provisioningSessionType` will be "DOWNLINK". The entry point `locator` will be a "http://" URL followed by the
...
1. Using a bogus provisioning session id, i.e. anything except the provisioning session id returned by the `m1-session` command,
   ...
   For example, if the provisioning session id is 0c5cc9e6-a6dd-41ed-ae90-e781c44d0f0c, then this test uses a provisioning session id string that does not match that id.
