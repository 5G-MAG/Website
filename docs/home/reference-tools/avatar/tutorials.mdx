...
export const projectVideos = youtubePlaylists.projects?.avatar?.videos || [];
...
- Check [Resources](./resources) for the current repository list and status; these repositories are private today, see [Early Access](/early-access) to request access.
