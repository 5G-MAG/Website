...
[Standards: Content Delivery Protocols](/standards/content-delivery)
...
:::note[Audit baseline]
This describes each repository's release branch only &mdash; `main` for the MBS User Services repositories (rt-mbs-function, rt-mbs-transport-function, rt-mbs-client, rt-mbs-application, rt-libflute) and for the Open5GS fork, and `5mbs` for the srsRAN Project and srsRAN 4G forks &mdash; each read at its own most recent commit as of 2026-08-18 (dates range from 2025-10-02 for the gNB fork to 2026-08-07 for the most recently updated User Services repositories). Of these, rt-mbs-client and rt-mbs-application are private today; see [Early Access](/early-access) to request access if you want to check this audit's claims against their source yourself. It deliberately excludes `feature/mbs-compliance-fixes` and other `development`/feature branches, which carry substantial fixes and new functionality that have not reached a release branch. This is a source-level reading of the code against the specification text; it is **not** backed by a live end-to-end test of the full delivery chain on these branches. Several defects found below &mdash; in particular the missing GTP-U PDU Session Container on the N3mb downlink and the complete absence of MCCH/SIB20 broadcast signalling on both RAN forks &mdash; mean the chain from the MB-UPF through the gNB to a real UE does not work today on these release branches, regardless of what earlier live testing on unreleased branches may have shown.
:::
...
- Standards: [Content Delivery Protocols](/standards/content-delivery) &mdash; the FLUTE transport under the Object distribution method, and why it is FLUTE version 1
...
- Implementation Blueprints: [MBS Broadcast RAN acquisition](/tech/5g-mbs/analysis-mbs-broadcast-ran#implementation-blueprint), [MBS Multicast RRC_INACTIVE RAN acquisition](/tech/5g-mbs/analysis-mbs-multicast-inactive-ran#implementation-blueprint), [MBS Multicast mobility](/tech/5g-mbs/mobility-mbs-multicast#implementation-blueprint), and [MBS Service Layer provisioning-to-announcement](/tech/5g-mbs/mbs-service-layer#implementation-blueprint) &mdash; the step-by-step procedures this tool implements
