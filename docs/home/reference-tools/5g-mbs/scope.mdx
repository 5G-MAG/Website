...
The tools currently implement the **MBS Broadcast** delivery mode end-to-end. For **MBS Multicast**, only the MB-SMF/MB-UPF data path (N6mb ingest, shared GTP-U delivery) exists; the session-join control plane (UE session join via the SMF, PTP/PTM switching, group paging) is not yet implemented in any component of the chain.
:::
