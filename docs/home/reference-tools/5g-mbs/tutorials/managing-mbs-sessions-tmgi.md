...
The call flow below shows the messages exchanged when a Broadcast MBS Session is created.

![Call flow for creating a Broadcast MBS Session, showing the AF/AS request to the MB-SMF and the resulting PFCP and NGAP signalling](/assets/images/5mbs/MBS_Broadcast_Session_Create_call_flow.png)
...
time is set in UTC and every time a TMGI is created the _expiration time_ is set to two hours.
The `MB-SMF TMGI Service API` using the _TMGI Allocate Service operation_ can be used to refresh the _expiration time_ of
...
1. If the TMGI has been previously allocated using the `MB-SMF TMGI Service API`, an MBS Session must be created sending
   ...
   operation_ with `tmgiAllocReq` property set to _false_ (this way only an MBS Session is created). The request must
...
After sending the request for the MBS Session creation, some side effects occur. The list below marks which are currently implemented and observable, and which are not yet implemented:

- **Implemented (observable):** MB-SMF and MB-UPF start the PFCP Session Establishment with the `MB-SMF PFCP Session Establishment extensions` using
...
- **Not yet implemented:** MB-SMF sends the AMF MBS Broadcast ContextCreate with the `AMF MBS Broadcast Service API` using _MBS Broadcast
  ContextCreate Service operation_
- **Not yet implemented:** AMF sends the NGAP BROADCAST SESSION SETUP to the gNBs with the `AMF NGAP extensions` using _BROADCAST SESSION SETUP
...
With this method, the AF/AS will ask the MB-SMF to refresh an existing TMGI. This method is only accessible through
...
The response of the MB-SMF contains the allocated TMGI as MBS Session identifier and also the SSM specified in the
request:
...
- [Initial support of MBS in the 5GC](./mbs-in-5gc): use these session and TMGI operations to send multicast traffic through the core.
- [MBS Transport Function (MBSTF) Testing](./mbstf): deliver media over an MBS session using FLUTE.
