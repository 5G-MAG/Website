...
A 5G Core with UDP tunnelling should be running for these examples (unless using the _netcat_ package to fake an MB-UPF UDP tunnel) with at least the NRF, SCP, MB-SMF, MB-UPF and MB-AMF Network Functions (see the instructions in the [Open5GS 5MBS branch](https://github.com/5G-MAG/open5gs/tree/5mbs) README).
...
This tutorial configures the MBSTF with examples of MBS Distribution Session types. This tutorial will also cover optional integration with the MB-SMF and MB-UPF.

**What this covers:** four distribution scenarios, exercised in Steps 5 to 8: Single-shot PULL, Single-shot PUSH, Streaming PULL (DASH) and Streaming PUSH (DASH).

**What you will build / outcome:** a running MBSTF that packages media objects into a FLUTE session, which you can observe in Wireshark leaving towards the MB-UPF tunnel.

**Setup vs tests:** Steps 1 to 4 are setup (create or fake the UDP tunnel, start the media server, run the MBSTF, and start Wireshark). Steps 5 to 8 are the actual distribution tests. For the UDP tunnel in Step 1 you can either use a real MB-SMF/MB-UPF (Step 1a) or fake the receiving end with `netcat` (Step 1b) if you only want to inspect the MBSTF output in isolation.

In the sequence diagrams that follow, the green-shaded boxes highlight the steps that the current tutorial actions actually perform (the command-line calls stand in for what the MBSF would do in a full deployment).
...
To use this you will need a 5G Core with an MB-UPF and an MB-SMF that are both capable of allocating an ingress tunnel at reference point Nmb9. The commands below clone, build and launch the MBS-capable Open5GS core network functions (or use the tmux script instead).
...
You can create an MBS Session with the Insomnia collection or with curl. For the latter, copy this Nmbsmf_MBSSession _CreateReqData_ JSON object into a file called `create-mbs-session.json`:
...
Then send this file to the MB-SMF to request that it creates a new multicast MBS Session (sequence steps 4 to 7):
...
On success the MB-SMF creates the session and returns the allocated TMGI and the ingress tunnel details. The response will look similar to this:
...
This requires the `netcat` or `nc` command from the _netcat_ package (_nmap-ncat_ on RHEL based systems).
...
When configured to operate in one of the pull-based object acquisition methods, the MBSTF requires an HTTP server from which to request the objects. For the purposes of these tutorials, a simple HTTP server based on the _NodeJS_ [_Express_](https://expressjs.com/) module is used, configured to serve some simple media objects. This simulates a media server or CDN that would be provided by the MBS Application Provider (AF/AS).

You can install and run the _Express_ simple media server directly or via Docker from the rt-mbs-examples repository:
...
- In the _Analyze_ menu, select _Decode As..._ to open the "Decode As..." dialog.
...
- **Dissect the multicast packets carried inside the reference point Nmb9 unicast tunnel.** If a rule does not exist for the UDP port `5000` (the port to be used for the multicast) then create a new rule for a "UDP port", set the port number to `5000` and the Current decoding to `ALC`.
- Select the _Save_ or _OK_ button to close the dialog. Saving will store the rules for next time Wireshark is started.
...
:::note[FLUTE in one paragraph]
FLUTE (File Delivery over Unidirectional Transport) sends files one-way, with no return channel from the receiver. Each file is a Transport Object with a Transport Object Identifier (TOI). TOI 0 is special: it carries the FDT (File Delivery Table) Instance, a manifest that lists the other objects being sent and their content locations. So in the Wireshark captures below, look for TOI 0 (the FDT manifest) followed by TOI 1, 2, 3 and so on (the actual media objects).
:::
...
- _Express_ server from Step 2
- the MBSTF from Step 3
- Wireshark from step 4

...perform the following actions to test a single shot distribution from _PULL_ requested media objects.
...
Then send the _DistSession_ JSON object in `DistSession-PULL-request.json` to the MBSTF to configure it (sequence step 6):
...
The _Wireshark_ packet capture (sequence step 11) will look like:
...
This shows the FLUTE FDT Instance `File` entry for the first object pulled from the _Express_ server. Examining the
packet it can be noticed that:
...
2. The next (inner) pair of IP and UDP headers show that this encapsulated packet is from 127.0.0.1:5000 to multicast
   address 232.0.0.1:5000 &#x2781;, as requested in the _DistributionSession_ creation request to the MBSTF.
3. The packet contents are a FLUTE packet for **Transport Session Identifier** 0 and **Transport Object Identifier** 0 (**"
   ...
   &#x2784;. The "http://127.0.0.2" prefix is the one requested by the _objDistributionBaseUrl_ field in the
   _DistributionSession_ configured in the MBSTF, and has replaced the origin prefix of "http://127.0.0.1:3004" (
   _objIngestBaseUrl_ field).
4. The next packet contains the same multicast packet &#x2786; sent from the MB-UPF to the gNodeB using GTP-U tunnel
   encapsulation.

This next screenshot shows the FLUTE packet for **TOI 1**, the ingested media object _object1_ itself.
...
...perform the following actions to test a single shot distribution for _PUSH_ delivered media objects.
...
Then send the _DistSession_ object to the MBSTF to configure it (sequence step 6):
...
Take note of the _objIngestBaseUrl_ prefix URL for pushing the media objects to, e.g.:

```bash
push_prefix_url="http://127.0.0.1:33945/"
```

Objects can then be sent to be packaged and sent via the multicast MBS Session (sequence steps 9 and 10). For example, to
send the `meson.build` file from the rt-mbs-transport-function repository that was cloned earlier, perform an HTTP PUT
of the file to a URL with the prefix noted earlier:
...
This shows the FDT for the pushed file. This is encapsulated in the same way as in step 5 of the pull-based object
acquisition example. The _Content-Location_ field in the FDT `File` entry shows it using the _objDistributionBaseUrl_
followed by the path that the object was `PUT` to following the _objIngestBaseUrl_.
...
This packet (packet number 9 in the above image), containing the first bytes of the `meson.build` file, is followed by
the rest of the file in packet number 11.
...
...perform the following actions to test a streaming distribution from _PULL_ of a DASH MPD.
...
This MBSTF Distribution Session is currently _INACTIVE_ so no MPD will be fetched and no FLUTE stream will be emitted.

- Note that the present implementation of the MBSTF only supports `live-profile` DASH manifests.

To push the _DistSession_ to the MBSTF to configure it (sequence step 6), use the following command:
...
them as a FLUTE stream, change the state to _ACTIVE_ (sequence step 22). This can be done using the
following command:
...
become available, each media segment for each _Representation_ declared for the DASH presentation in its MPD.
...
a `File` entry for the MPD transmission object (**TOI 1**). The _objIngestBaseUrl_ from the Distribution Session
configuration has been substituted for the _objDistributionBaseUrl_ in the FDT File entry, resulting
...
_objDistributionBaseUrl_ substitution happens to this URL too, resulting in the advertised path
...
To stop the MBSTF DistSession, change its state to the _INACTIVE_ state (sequence step 48). This can be done with
...
the _INACTIVE_ state. A new FLUTE stream can now be started by changing the MBS DistSession state to _ACTIVE_ again, if
so desired.
...
This configuration behaves in a similar manner as the _PULL_ _STREAMING_ configuration in Step 7 except that:

1. The MPD is the only file whose distribution URL, as given in the _Content-Location_ attribute in the distributed FDT
   Instance `File` entry, is subject to substitution with the _objDistributionBaseUrl_ value from the configuration (it
   is the only file pushed to the _objIngestBaseUrl_ URL prefix). All initialization segments and media segments will be
   distributed with the _Content-Location_ attribute set to the original media server URL.
...
...perform the following actions to test a streaming distribution from the _PUSH_ of a DASH MPD.
...
When pushed as a configuration for the MBSTF, this will configure a Distribution Session that will provide an ingest URL
and will wait for a Service Entry Point (DASH manifest) document to be _PUT_ to the ingest URL. The ingest URL is the
concatenation of _objIngestBaseUrl_ and _objAcquisitionIdPush_ from the response.

Note that the present implementation of the MBSTF only supports `live-profile` DASH manifests as the Service Entry Point
document.
...
The MBSTF response (sequence step 7) should look like (the _objIngestBaseUrl_ field will be different):
...
In this example response, the ingest URL, formed from _objIngestBaseUrl_ and _objAcquisitionIdPush_, would be
...
The MBSTF will now wait for a live profile DASH MPD to be _PUT_ to the ingest URL (`http://127.0.0.1:12345/manifest.mpd`
from the example) with _Content-Type_ `application/dash+xml`. This push from the AP is simulated below (sequence step 9).

To push a _live-profile_ MPD, use the command:
...
Once pushed the MBSTF will add the MPD to the distribution FLUTE stream and proceed to pull the initialization segments
and media segments as happened in Step 7 above for the _PULL_ _STREAMING_ Distribution Session.

The Wireshark capture will show that the MPD has a _Content-Location_ in its FDT File entry which is the concatenation
of the _objDistributionBaseUrl_ and _objAcquisitionIdPush_. All other objects sent on the FLUTE stream will have
_Content-Location_ entries showing a URL from the origin media server. For this example they will all begin with
`https://livesim2.dashif.org/livesim2/WAVE/vectors/cfhd_sets/12.5_25_50/t1/2022-10-17/`.

The following Wireshark screenshot shows the FDT File entry for the MPD with a _Content-Location_ URL
of "http://127.0.0.2/manifest.mpd". This comes from the _objDistributionBaseURL_ concatenated with the
_objAcquisitionIdPush_ value.
...
The next Wireshark screenshot above shows the FDT File entry for the first initialization segment after an MPD has been
pushed. Because the MPD included a _BaseURL_ element, directing the rest of the fetches to the original media server,
the _Content-Location_ URL is the URL the initialization segment was pulled from.
...
- [MBS Function (MBSF) Operation & APIs](./mbsf): drive the MBSTF from the MBSF over Nmb10, instead of by hand.
- [Managing MBS Sessions and TMGIs](./managing-mbs-sessions-tmgi): the MB-SMF session operations behind Step 1a.
