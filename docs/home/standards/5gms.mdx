...
import { FIVEGMS_SPECS, FIVEGMS_LAYER_ORDER } from '@site/src/data/specs/5gms';
...
The specifications in scope, filterable by the layer they belong to: the 5GMS specifications themselves (architecture in TS 26.501, protocols and APIs in TS 26.512 and TS 26.510, profiles and codecs in TS 26.511), the two companion specifications defining the generic UE data collection framework that 5GMS reporting can feed into, and the two 5G Core service-based APIs the unicast (5GMSd/5GMSu) path relies on. Search matches the specification number, its title and any scope note.

<SpecIndex specs={FIVEGMS_SPECS} layerOrder={FIVEGMS_LAYER_ORDER} />
...
- **Specification: [CTA-5004](https://web.archive.org/web/20260725054240/https://cdn.cta.tech/cta/media/media/resources/standards/pdfs/cta-5004-final.pdf) - Web Application Video Ecosystem - Common Media Client Data**
