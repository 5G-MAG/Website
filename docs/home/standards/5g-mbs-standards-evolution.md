...
CR-level detail uses this project's own [3GPP Explorer](https://github.com/5G-MAG/Standards2Deployment) tool against 3GPP's public Change Request database export. Every CR cited above is confirmed `agreed` at working-group level and `approved` at TSG level in that export, cross-checked against the work item acronym its own portal page names. No 3GPP delegate login is needed for any of this — the CR database itself is public; only per-CR document *text* sits behind the delegate-only portal.
...
**Release 16** is checked the same way as every other release here: a work item portal search for Release 16 with no keyword restriction, filtered to every item whose name or acronym contains "multicast," "broadcast" or "MBMS" (9 of 719 Release 16 work items matched); each was read individually, and each is either legacy MBMS work or the separate LTE-based 5G Broadcast feature, not 5G MBS. TS 26.502's Release 16 gap is confirmed against 3GPP's own archive directory listing for the specification, which lists no `g`-prefixed (Release 16) version between `26502-200.zip` and `26502-h00.zip`.
