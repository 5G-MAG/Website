...
The Update operation for an MBS User Data Ingest Session is performed by using an HTTP *PUT* to the API URL ending in `.../nmbsf-mbs-ud-ingest/v1/sessions/{mbs_user_data_ing_session_id}`. The body of the request is a modified *MBSUserDataIngSession* structure.
...
To do this an HTTP *GET* request is sent to the API URL ending in `.../nmbsf-mbs-ud-ingest/v1/sessions/{mbs_user_data_ing_session_id}`.
...
To remove an MBS User Data Ingest Session the Delete operation is used. This is done using an HTTP *DELETE* request to the API URL ending in `.../nmbsf-mbs-ud-ingest/v1/sessions/{mbs_user_data_ing_session_id}`.
