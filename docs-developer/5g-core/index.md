...
sidebar_position: 5
description: Overview of the reference implementation for media applications that consume 5G Core PCF, NEF, and BSF services per 3GPP Release 16/17.
---
...
Reference implementation for media applications that consume 5G Core (5GC) network services directly, including policy authorisation, network event exposure, and content steering, using the interfaces defined for media applications in 3GPP Release 16 and 17.
</div>
...
:::tip[Where to start]
Read the [Scope](./scope), then follow the [Using libscPCF](./tutorials/using-libscPCF) tutorial, which is the most complete worked example (it also exercises the BSF along the way).
:::
...
<iframe loading="lazy" width="60%" height="560" src="../../docs/Reference_Tools_5GC_Service_Consumers.pdf" title="5G Core Service Consumers reference tools overview slide deck"></iframe>
