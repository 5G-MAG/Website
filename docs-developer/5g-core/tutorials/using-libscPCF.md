...
**What you will do:** run the `pcf-policyauthorization` tool to request a QoS policy for a device, and read the debug output to confirm the tool found the PCF (via the BSF) and created an Application Session Context.
...
Note: This will only work if you have defined a Policy and Charging Control (PCC) Rule for 5G QoS Identifier (5QI) 2 (video) defaults. A 5QI is a standardised value that maps to a set of QoS characteristics (for example priority, latency and packet loss), and a PCC Rule is the policy the PCF applies to a traffic flow.
...
The debug log below is a full example run. Note that this capture uses the device (UE) IP address `10.45.0.2`, which differs from the `12.34.56.78` used in the command examples above; substitute whichever address the 5G Core assigned to your device. As you read the log, the lines to look for are: `Discovering PCF for UE...`, then `Found PCF...` (the BSF lookup succeeded), `Requesting AppSessionContext...`, `POST response: status = 201` and `AppSessionContext created...` (the QoS policy request was accepted), and finally `Awaiting notifications...`. At that last line the tool stays running to report events; stop it with Ctrl+C.
...
- [Using libscMBSMF](./using-libscMBSMF): the MB-SMF service consumer library for MBS sessions.
