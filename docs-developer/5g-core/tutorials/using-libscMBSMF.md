...
<h1>Using the 5GC Service Consumer libraries: libscMBSMF</h1>
</div>
</div>
...
This collection of reusable service consumer libraries is designed to talk to the 5G Core Network Functions using some of their defined service interfaces. The command line tools below are provided to demonstrate the use of these service consumer libraries.

**What you will do:** run the `tmgi-tool` and `mbs-service-tool` command-line tools against a running MB-SMF and read their debug output to confirm that a TMGI, and then an MBS Session, were created.

:::note[Reading the debug logs]
The examples below use the `-e debug` option, so they include verbose output. The hex dumps (lines starting with `0000:`) are the raw HTTP/2 frames on the wire; you can skip them and focus on the human-readable log lines. Below each long log, look for the confirmation lines highlighted in the notes.
:::

## Setup the relevant Open5GS network functions

We recommend a local installation of Open5GS following the instructions provided [here](../../3gpp-platforms/tutorials/5gnetwork).

Note that the **TMGI Allocation/Deallocation tool** and the **MBS Service tool** require the MBS components under the `5mbs` branch of 5G-MAG's Open5GS repository. This can be cloned with: `git clone --recurse-submodules -b 5mbs https://github.com/5G-MAG/open5gs.git ~/open5gs`. The MBS Transport Function can be installed from the [rt-mbs-transport-function](https://github.com/5G-MAG/rt-mbs-transport-function) repository.
...
Success looks like the `TMGI[...] ... created` line above, showing the allocated `mbs_service_id` (here `CE2E09`) and its expiry time, followed by `TMGI operation finished, exiting...`. On the MB-SMF side, the matching confirmation is `[Added] Number of TMGIs in SMF is now N` and a `STATUS [200]` response.
...
Success looks like the `TMGI[...] ... deleted` line, and on the MB-SMF side `[Removed] Number of TMGIs in SMF is now N` with a `STATUS [204]` response.
...
The key confirmation lines in the tool output are `MBS Session 1 [...] created`, the allocated `TMGI = EA7DDD (PLMN = 000000)`, and the `UDP Tunnel = 127.0.0.7:37259` (the ingress tunnel the MB-UPF allocated). The tool then prints `Awaiting notifications...` and stays running so it can report events for the session; stop it with Ctrl+C when you are done.
