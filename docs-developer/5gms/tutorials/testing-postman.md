...
our routes or payloads. The first variables we need to define are the `m1_url`, `m5_url`, `maf_url` variables. The `maf_url` points at the MAF collection: a set of 5G-MAG-specific endpoints that are not part of the [TS 26.512](https://www.3gpp.org/dynareport/26512.htm) standard (see [Using custom endpoints](#using-custom-endpoints) below). For that
reason,
...
running
the Application Function in a local network. Note that these URLs also need to contain the port. The configuration
