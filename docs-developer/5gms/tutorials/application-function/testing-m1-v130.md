...
hide_title: true
sidebar_position: 7
description: Manual test procedures for the M1 interface of the 5GMSd Application Function v1.3.0 to v1.4.0, including consumption reporting configuration.
---
...
1. Create a single Provisioning Session with Content Hosting Configuration containing multiple entry points
   ...
   ```bash
   ~/rt-5gms-application-function/install/bin/m1-session new-stream -e MyAppId -a MyASPId -n 'BBC Test Card' 'https://rdmedia.bbc.co.uk/testcard/vod/' 'manifests/avc-full.mpd' 'manifests/avc-full.m3u8'
   ```
